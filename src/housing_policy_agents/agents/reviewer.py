...
def offline_review(report: DraftReport, ledger: SourceLedger) -> AdversarialReview:
    ...
    for section in report.sections:
        for paragraph in section.paragraphs:
            weak_sources = [record.source_id for record in ledger.values() if record.source_id in paragraph.citation_ids and record.tier >= 5]
            if weak_sources:
                findings.append(ReviewFinding(
                    severity="major",
    ...
    return AdversarialReview(
        ...
        release_recommendation=ReleaseRecommendation.REVISE if mandatory else ReleaseRecommendation.APPROVE_WITH_CAVEATS,
        reviewer_notes=validation.warnings,
    )


async def review_report(report: DraftReport, ledger: SourceLedger, evidence: object, context: RunContext) -> AdversarialReview:
    ...
    result = await Runner.run(
        agent,
        json.dumps({"draft": report.model_dump(mode="json"), "sources": [item.model_dump(mode="json") for item in ledger.values()], "evidence": evidence}, indent=2),
